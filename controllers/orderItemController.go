...
func GetOrderItems() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil{
			c.JSON(http.StatusBadRequest, gin.H{"error":"error occurred while listing ordered items"})
			return
		}
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	//projectStage manages the fields to be returned to the frontend or the next stage
	projectStage := bson.D{
		{"$project", bson.D{
			{"id", 0}, // id not[0] to go to next stage
			{"amount", "$food.price"}, // amount to go to next stage as food.price
			{"total_count", 1}, //proceed [1]
			{"food_name", "$food.name"},
...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil{
			c.JSON(http.StatusBadRequest, gin.H{"error":"error occurred while listing ordered item"})
			return
		}
